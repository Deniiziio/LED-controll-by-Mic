...
#define LED_TOTAL 180  //Change this to the number of LEDs in your strand.
...
#define BUTTON_3  11   //Button 3 toggles shuffle mode (automated changing of color and visual)
int CLK = 13; //for trimpot
int DAT = 12; //for trimpot
int BTN = 11; //for trimpot
int buttonState;         
...
float maxVol = 250;   //Holds the largest volume recorded thus far to proportionally adjust the visual's responsiveness.
float minVol = 80;    //Holds the smallest volume recorded thus far to proportionally adjust the visual's responsiveness.
...
//For Traffic() visual
int8_t pos[LED_TOTAL] = { -6};    //Stores a population of color "dots" to iterate across the LED strand.
...
void setup() {    //Like it's named, this gets ran before any other function.
  ...
  pinMode(BUTTON_1, INPUT); pinMode(BUTTON_2, INPUT); pinMode(BUTTON_3, INPUT);//Defines the buttons pins to be input.
  digitalWrite(BUTTON_1, HIGH); digitalWrite(BUTTON_2, HIGH); digitalWrite(BUTTON_3, HIGH);  //Write a "HIGH" value to the button pins.
...
void loop() {  //This is where the magic happens. This loop produces each frame of the visual.
  Soundsens();  //  Sets a threshold for volume.
  if (volume < avgVol / 2.0 || volume < minVol) volume = 0;
  ...
  if (gradient > thresholds[palette]) {//This is where "gradient" is modulated to prevent overflow.
  ...
  if (volume - last > 10) avgBump = (avgBump + (volume - last)) / 2.0;//If there is a decent change in volume since the last pass, average it into "avgBump"
...
void Soundsens(){
   ...
   {
      sample = analogRead(0);
      if (sample < 256)  // toss out spurious readings
      {
         if (sample > signalMax)
         {
